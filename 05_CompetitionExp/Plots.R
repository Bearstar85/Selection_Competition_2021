...
setwd("~/Documents/R/Andersson_etal_2021/05_CompetitionExp")
dir()
...
#Import data 
PAM <- read.csv2("Data/PAMComp.csv", header=TRUE)
...
FigPAM <- ggplot(AvaragesPAM, aes(t, mean)) +
  ...
  #scale_fill_manual(values=c("#008000", "#2846FF", "#6aa84f")) +
...
dev.copy(pdf, "Figures/FigPAM.pdf")
dev.off()
...
write.csv(AvaragesRedBlue, "Table/AvaragesRedBlue.csv")
...
dev.copy(pdf, "Figures/FigRedBlue.pdf")
dev.off()

#Next RFU####

dir()
RFU <- read.csv2("Data/RFUComp.csv", header=TRUE)
...
dev.copy(pdf, "Figures/FigRFU.pdf")
dev.off()

#Next predicted growth rates based on drc####

dir()
RateMdrc <- read.csv("Data/RateMdrc.csv", header=FALSE)

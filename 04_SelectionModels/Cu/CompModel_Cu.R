...
setwd("~/Documents/R/Andersson_etal_2021/04_SelectionModels/Cu")
dir()
...
ggplot(data = df.melted, aes(x = Strain, y = value, color = variable)) +
  geom_point()
#For Cu it looks like the observed inhibition is stronger than predicted across all strains
...
#There is poor correlation between observed and predicted inhibition in strains, 
#As in Ag plot, the T. baltica strain that crashes has strong influence on this (lower right corner) 
...
CuTable <-  format(myData3, trim = FALSE, digits = 2)

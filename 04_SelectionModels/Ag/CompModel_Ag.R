...
setwd("~/Documents/R/Andersson_etal_2021/04_SelectionModels/Ag")
dir()
...
myData <- read.csv("PRvaluesAg.csv", header=FALSE)
head(myData)
myData2 <- read.csv2("Strain_observations.csv", header=TRUE)
Species <- c("SM", "SM", "SM", "SM", "TB", "TB", "TB", "TB")
myData3 <- cbind(myData, Species, myData2$Growth_rate, myData2$Ag_inhibition)
...
#Note the outlier is the T. baltica strain that crashed during experiment
...
AgTable <-  format(myData3, trim = FALSE, digits = 2)

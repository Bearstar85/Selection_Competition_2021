...
setwd("~/Documents/R/Andersson_etal_2021/04_SelectionModels/ModelResults/Ag")
dir()
...
myData <- read.csv("Input/PRoutcomeAg.csv", header=FALSE)
head(myData)
...
#First drc bases
myData4 <- read.csv("Input/StrainMdrc_Ag.csv", header=FALSE)
...
myData5 <- read.csv("Input/StrainMO_Ag.csv", header=FALSE)
...
Ag <- subset.data.frame(myData3, grepl("Ag", myData3$Metal))

#Plot data to make sure it looks ok
ggplot(data = Ag, aes(x = t, y = Area)) +
...
Ag <- separate(data = Ag, col = Index, into =c("Species", "Model"), sep = "_", remove = TRUE, convert = FALSE)
Ag

Ag_SM <- subset.data.frame(Ag, grepl("SM", Ag$Species))
Ag_TB <- subset.data.frame(Ag, grepl("TB", Ag$Species))

#and add up all vector models####

#This is dangerous and risk of misalignment must be doubled-checked (e.g. last plot)
Ag_ratios <- cbind(Ag_SM, Ag_TB)
colnames(Ag_ratios) <- c("Species_SM", "Model_SM", "Metal_SM", "t_SM", "Area_SM", "Species_TB", "Model_TB", "Metal_TB", "t_TB", "Area_TB")
plot(Ag_ratios$t_SM~Ag_ratios$t_TB)

#Looks ok, now we can calculate biomass ratios
Ag_ratios$Ratio <- Ag_ratios$Area_SM/Ag_ratios$Area_TB
Ag_ratios
...
Keep <- c("Exp")
Ag_ratios2 <- filter(Ag_ratios, Model_SM %in% Keep)
Ag_ratios2$Model_SM <- recode_factor(Ag_ratios2$Model_SM, 
                                     Exp = "Observation")
Ag_ratios2$Species_SM <- NULL
Ag_ratios2$Area_SM <- NULL
Ag_ratios2$Species_TB <- NULL
Ag_ratios2$Model_TB <- NULL
Ag_ratios2$Metal_TB <- NULL
Ag_ratios2$t_TB <- NULL
Ag_ratios2$Area_TB <- NULL

colnames(Ag_ratios2) <- c("Model", "Metal", "t", "Ratio")

#REMOVING SelectionMO from plotting
Ag_ratios3 <- rbind(Ag_ratios2, SelectionM)
Ag_ratios3$t <- as.numeric(as.character(Ag_ratios3$t))
Ag_ratios3$Ratio <- as.numeric(as.character(Ag_ratios3$Ratio))
                          
Fig3.B <- ggplot(Ag_ratios3, aes(t, Ratio)) +
  ...
  geom_point(data = subset(Ag_ratios3, Model == "Observation"), shape = 1, size = 4) + 
  #geom_smooth(mapping = aes(x = t_SM, y = (Ratio), color = Model_SM), method = "auto") +
  scale_color_manual(values=c("#000000", "#828282", "#cccccc"), aesthetics = c("colour", "fill"), guide=FALSE) +
  ...
  labs (x="Time (days)", y=("Relative biomass (SM/TB)"), title = "B) Ag") +
...
dev.copy(pdf, "Results/Fig3B.pdf")
dev.off()
...
FigDRC.A <- ggplot(data = longDataDrc, aes(x = Var1, y = value, fill=Var2)) + geom_area(position='fill') +
  labs (x="Time (days)", y=("Relative biomass"), title = "B) Ag") +
...
dev.copy(pdf, "Results/FigDRC.pdf")
dev.off()
...
dev.copy(pdf, "Results/FigO_A.pdf")
dev.off()
...
RateSMdrc <- as.data.frame(cbind(t, rep("SM", each = 101), rep("Ag", each = 101), (log(SM_strain2_v/lag(SM_strain2_v))/(t - lag(t)))))
RateTBdrc <- as.data.frame(cbind(t, rep("TB", each = 101), rep("Ag", each = 101), (log(TB_strain2_v/lag(TB_strain2_v))/(t - lag(t)))))                         
...
RateSMO <- as.data.frame(cbind(t, rep("SM", each = 101), rep("Ag", each = 101), (log(SM_strain4_v/lag(SM_strain4_v))/(t - lag(t)))))
RateTBO <- as.data.frame(cbind(t, rep("TB", each = 101), rep("Ag", each = 101), (log(TB_strain4_v/lag(TB_strain4_v))/(t - lag(t)))))                       

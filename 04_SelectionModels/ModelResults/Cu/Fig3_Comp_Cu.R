...
setwd("~/Documents/R/Andersson_etal_2021/04_SelectionModels/ModelResults/Cu")
dir()
...
myData <- read.csv("Input/PRoutcomeCu.csv", header=FALSE)
head(myData)
...
#First drc bases
myData4 <- read.csv("Input/StrainMdrc_Cu.csv", header=FALSE)
...
myData5 <- read.csv("Input/StrainMO_Cu.csv", header=FALSE)
...
Cu_ratios3 <- rbind(Cu_ratios2, SelectionM)
...
Fig3.D <- ggplot(Cu_ratios3, aes(t, Ratio)) +
  ...
  scale_color_manual(values=c("#000000", "#B85633", "#cccccc"), aesthetics = c("colour", "fill"), guide=FALSE) +
...
dev.copy(pdf, "Results/Fig3D.pdf")
dev.off()
...
dev.copy(pdf, "Results/FigDRC.D.pdf")
dev.off()
...
dev.copy(pdf, "Results/FigO.D.pdf")
dev.off()

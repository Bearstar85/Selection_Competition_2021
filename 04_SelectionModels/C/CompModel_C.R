...
setwd("~/Documents/R/Andersson_etal_2021/04_SelectionModels/C")
dir()
...
#Note that i have used the Cu script for the control and modify it on line 65 and 66 to remove effects of inhibition values. This was a simplifying shortcut.

#Import data (a bit messy)
myData <- read.csv("PRvaluesCu.csv", header=FALSE)
head(myData)
myData2 <- read.csv2("Strain_observations.csv", header=TRUE)
Species <- c("SM", "SM", "SM", "SM", "TB", "TB", "TB", "TB")
myData3 <- cbind(myData, Species, myData2$Growth_rate, myData2$Cu_inhibition)
...
ggplot(data = df.melted, aes(x = Strain, y = value, color = variable)) +
  geom_point()
#For Ag it looks like the observed inhibition is stronger than predicted across all strains
...
#Now for the predictive models (modified for Control!!!!)#####
Avarages3
Avarages3$PRuP <- (Avarages3$Growth)
Avarages3$PRuO <- (Avarages2$Growth)
Avarages3$PR <- NULL
Avarages3$Low <- NULL
Avarages3$High <- NULL
...
#done use cat to combine output files
...
Strain <- myData3
Strain$PRuP <- (Strain$Growth)
Strain$PRuO <- (Strain$Growth)

#Create individual parameter files
for (name in levels(Strain$Strain)){
  #Subset data based on ID
